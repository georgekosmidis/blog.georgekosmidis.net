...
foreach (var item in blogPostsResponse.BodyAsType.Items.OrderBy(x => x.DatePublished))
{
    ...
    foreach (Match match in Regex.Matches(item.ContentHtml, "https:\\/\\/gist\\.github\\.com\\/georgekosmidis\\/([^.]+)\\.js\\?file=([^\"]+)"))
    {
        ...
        Match rawCodeMatch = Regex.Match(jsScript.BodyAsString, $"https:\\/\\/gist\\.github\\.com\\/georgekosmidis\\/{match.Groups[1].Value}\\/raw\\/([^\"]+)\\/{match.Groups[2].Value}");
    ...
    var migratedPost = new MigratedPost(slang, title, description, datePublished, dateModified, image, tags);
